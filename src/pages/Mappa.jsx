...
export default function Mappa({ dark, onToggle }) {
  ...
  useEffect(() => {
    ...
    MOCK_MARKERS.forEach((m) => {
      ...
      el.style.cssText = `width:14px;height:14px;border-radius:50%;background:${TIPI[m.tipo].color};border:2px solid rgba(255,255,255,0.6);cursor:pointer;transition:transform 0.15s`
      ...
      markerRefs.current[m.id] = { marker, el, tipo: m.tipo }
    })
  ...
  useEffect(() => {
    Object.values(markerRefs.current).forEach(({ marker, el, tipo }) => {
      el.style.display = filtri[tipo] ? 'block' : 'none'
    })
  }, [filtri])
    ...
    <div className="h-screen flex flex-col bg-dark text-cream overflow-hidden">
      ...
      <nav className="flex-shrink-0 flex items-center justify-between px-6 py-3 border-b border-night-border bg-dark/90 backdrop-blur-sm z-10">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="font-sans text-sm text-cream">Mappa</span>
        </div>
      </nav>
      ...
      <div className="flex flex-1 min-h-0">
        {/* Sidebar */}
        <aside className="w-64 flex-shrink-0 border-r border-night-border bg-dark flex flex-col overflow-y-auto p-5 gap-6 z-10">
          <div>
